...
switch between Google Gemini (free), Groq (free), or OpenAI with one
...
def get_llm(provider: str | None = None):
    ...
    All providers use ``temperature=0`` for deterministic SQL generation.
    """
    provider = (provider or DEFAULT_PROVIDER).lower()
    ...
    raise ValueError(
        f"Unknown LLM_PROVIDER '{provider}'. Use one of: google, groq, openai."
    )
